...
import {
  ...
  findUserById,
  updateUserPassword,
} from "../modulars/adminAuthModule.js";
...
export const loginUser = async (req, res) => {
  try {
    ...
    // Add is_admin to the token payload
    const token = jwt.sign(
      {
        id: user.admin_id || user.user_id, // adapt if needed
        is_admin: user.is_admin === 1,     // make sure it's a boolean
      },
      secretKey,
      { expiresIn: "1d" }
    );
    ...
    return res.json({ status: "Admin logged in successfully", token });
  } catch (error) {
