...
//                                     if (typeof profile.profile_image !== 'string' && profile.profile_image?.preview) {
...
export default function ProfileSection({
    ...
    const handleSubmit = async () => {
        try {
            ...
            if (profile.profile_image && typeof profile.profile_image !== "string") {
                updatedProfile.profile_image = await uploadFile(
            ...
            if (profile.resume_file && typeof profile.resume_file !== "string") {
                updatedProfile.resume_file = await uploadFile(
                    profile.resume_file,
                    "resume_file"
                );
            }
            ...
            const response = await fetch(
                "http://localhost:9000/api/users/profile/update",
                {
                    method: "POST",
                    headers: {
                        "Content-Type": "application/json",
                    ...
                    {isEditing ? (
                        <div className="space-y-4">
                            ...
                            <div>
                                ...
                                <div className="flex items-center gap-3">
                                    {profile.resume_file &&
                                    typeof profile.resume_file === "string" ? (
                                        <a
                                            href={`http://localhost:9000/uploads/${profile.resume_file}`}
                                            target="_blank"
                                            rel="noopener noreferrer"
